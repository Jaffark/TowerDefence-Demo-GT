...
namespace TowerDefense.UI.HUD
{
	...
	public class GameUI : Singleton<GameUI>
	{
		...
		protected virtual void MoveGhostWithRaycastHit(RaycastHit raycast)
		{
            ...
            Debug.LogError("There is a Iplacement Area");
        ...
        public void Start()
        {
            SetRandomTowerAtStart();
        }
        /// <summary>
        /// Task Start level with Machine gun
        /// Build Random Tower will load random tower
        /// nNumberOfTowerToBuild variable is the number of Tower you would like to buid 
        /// </summary>
        /// 
        [Range(0,5)]
        public float nNumberOfTowerToBuild;
         void SetRandomTowerAtStart()
        {
            for(int i=0;i<nNumberOfTowerToBuild;i++)
            {
               Invoke("BuildRandomTower",0.25f);
            }
        }
        //Main logic that build random tower
        void BuildRandomTower()
        {
            
            state = State.Building;
            IPlacementArea[] area = GameObject.FindObjectsOfType<TowerPlacementGrid>();

            Tower towerToBuild = null;

            //You can also directly assign the machine gun without using foreach
            //You can also use towerToBuild = tower[0]; if you want
            //making it with for loop is also better then foreach 
            foreach (Tower tower in LevelManager.instance.towerLibrary)
            {
                
                towerToBuild = tower;//Since first tower is maching gun
                //There are different way you can assign any tower from Tower Library               
                break;
            }
            ...
            List<PlacementTile> possibleFitTiles = new List<PlacementTile>();
            List<int> areaId = new List<int>();
            List<IntVector2> gridAreaList = new List<IntVector2>();
            TowerFitStatus fits = TowerFitStatus.Overlaps;

            for (int j = 0; j < area.Length; j++)
            {
                this.m_CurrentArea = area[j];
                PlacementTile[] allTiles = m_CurrentArea.transform.GetComponentsInChildren<PlacementTile>();
                for (int i = 0; i < allTiles.Length; i++)
                {
                    PlacementTile anyTile = allTiles[i];
                    Vector3 pointToFit = anyTile.transform.position;
                    m_GridPosition = this.m_CurrentArea.WorldToGrid(pointToFit, m_CurrentTower.controller.dimensions);
                    fits = this.m_CurrentArea.Fits(m_GridPosition, m_CurrentTower.controller.dimensions);
                    if (fits == TowerFitStatus.Fits)
                    {
                        possibleFitTiles.Add(anyTile);
                        areaId.Add(j);
                        gridAreaList.Add(m_GridPosition);
                    }

                }
            }

            if (possibleFitTiles.Count <= 0)
            {
                CancelGhostPlacement();
                Debug.Log("No Possible To Fit Tiles");
                return;
            }
            //Taking random 
            int randomFitTile = UnityEngine.Random.Range(0, possibleFitTiles.Count);
            m_GridPosition = gridAreaList[randomFitTile];
            this.m_CurrentArea = area[areaId[randomFitTile]];


            m_CurrentTower.Show();
            m_GhostPlacementPossible = fits == TowerFitStatus.Fits && IsValidPurchase();
            m_CurrentTower.Move(this.m_CurrentArea.GridToWorld(m_GridPosition, m_CurrentTower.controller.dimensions),
            ...
            if (m_CurrentArea != null)
            {
                TowerFitStatus newfits = m_CurrentArea.Fits(m_GridPosition, m_CurrentTower.controller.dimensions);

                if (newfits == TowerFitStatus.Fits)
                {
                    // Place the ghost
