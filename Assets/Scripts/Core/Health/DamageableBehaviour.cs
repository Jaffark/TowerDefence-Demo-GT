...
namespace Core.Health
{
	...
	public class DamageableBehaviour : MonoBehaviour
	{
        ...
        void CreateSomeMoreL()
        {
            if (agentConfiguration==null || GetComponent<Agent>().isSpawnedOnDestroy)
                return;
            GetComponent<Agent>().isSpawnedOnDestroy = true;
            var poolable = Poolable.TryGetPoolable<Poolable>(agentConfiguration.agentPrefab.gameObject);
            if (poolable == null)
            {
                return;
            }
            var agentInstance = poolable.GetComponent<Agent>();
            agentInstance.transform.position =transform.position ;
            agentInstance.Initialize();
            
            agentInstance.SetNode(GetComponent<Agent>().m_CurrentNode);
            agentInstance.MoveToNode();
            agentInstance.transform.rotation = transform.rotation;
        }
