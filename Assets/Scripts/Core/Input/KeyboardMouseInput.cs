...
namespace Core.Input
{
	...
	public class KeyboardMouseInput : CameraInputScheme
	{
		...
		protected void DoScreenEdgePan()
		{
			...
			if (mouseInside && UnityInput.GetMouseButton(2))
			{
        ...
        protected void DoKeyboardPan()
		{
            ...
            currentScrollDelta = (int)UnityInput.mouseScrollDelta.y;

            if (lastScrollDelta != currentScrollDelta && scrollDirection==0)
            {
                lastScrollDelta = currentScrollDelta;
                              
            
                    lastStopScroll = Time.time;
                    scrollDirection = (int)UnityInput.mouseScrollDelta.y;
              
               
            }
            

            if (scrollDirection!=0  && Time.time >= lastStopScroll + 0.2f)
            {
                scrollDirection = 0;
			...
			// Down
			if (UnityInput.GetKey(KeyCode.DownArrow) || UnityInput.GetKey(KeyCode.S) || scrollDirection<=-1)
			{
			...
			// Up
			if (UnityInput.GetKey(KeyCode.UpArrow) || UnityInput.GetKey(KeyCode.W) || scrollDirection >= 1)
			{
